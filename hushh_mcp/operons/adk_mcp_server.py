...
import mcp.server.http as http_server     # For running as an HTTP server

# ADK Tool Imports
from google.adk.tools.function_tool import FunctionTool
from hushh_mcp.operons.detect_slots import detect_available_slots  # Example ADK tool
...
adk_tool_to_expose = FunctionTool(detect_available_slots)
...
async def run_mcp_http_server():
    """Runs the MCP server, listening for connections over HTTP."""
    # Use the http_server context manager from the mcp.server.http library
    async with http_server(host="0.0.0.0", port=8000) as server:
        print("MCP HTTP Server: Starting handshake with client...")
        await app.run(
            server,
            server,
        ...
        print("MCP HTTP Server: Run loop finished or client disconnected.")

if __name__ == "__main__":
    print("Launching MCP Server to expose ADK tools via HTTP...")
    try:
        asyncio.run(run_mcp_http_server())
    except KeyboardInterrupt:
        print("\nMCP Server (HTTP) stopped by user.")
    except Exception as e:
        print(f"MCP Server (HTTP) encountered an error: {e}")
    finally:
        print("MCP Server (HTTP) process exiting.")
# --- End MCP Server ---